...
> [!NOTE]
>
> 在 nopCommerce 中，論壇預設是停用的。若要啟用論壇，請前往 **設定 → 設定 → 論壇設定**，並勾選 **啟用論壇** 核取方塊。啟用後，「論壇」連結應會顯示在前台網站的選單中（預設佈景主題為頂部選單或頁尾）。
...
- 選擇論壇群組的 **顯示順序 (Display order)**。數值 1 代表位於清單的最上方。
...
此頁面支援多商店設定；這意味著可以為所有商店定義相同的設定，或針對不同商店設定不同的值。如果您想要管理特定商店的設定，請從多商店設定下拉式選單中選擇該商店名稱，並勾選左側所需的核取方塊，以設定其自訂數值。如需進一步詳情，請參閱 [多商店](xref:zh-Hant/getting-started/advanced-configuration/multi-store)。
...
- 勾選 **允許私人訊息** 核取方塊以啟用私人訊息功能。啟用後，將會顯示以下兩個設定：
...
- 在 **Active discussions feed count** 欄位中，設定「Active discussions」feed 所包含的討論數量。
