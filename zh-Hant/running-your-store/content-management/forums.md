...
論壇是一個線上討論網站，人們可以在其中以發布訊息的形式進行對話。一個論壇可能包含多個子論壇，每個子論壇又有數個內容頁面。
...
此頁面支援多商店設定；這表示您可以為所有商店定義相同的設定，或針對不同商店設定不同的值。如果您想要管理特定商店的設定，請從多商店設定下拉式清單中選擇該商店名稱，並勾選左側所需的核取方塊，以便為它們設定自訂值。如需進一步詳細資訊，請參閱 [多商店](xref:zh-Hant/getting-started/advanced-configuration/multi-store)。
...
- 您可以透過勾選 **啟用簽名 (Signature enabled)**，讓顧客有機會指定他們的個人簽名。
- 勾選 **顯示顧客發文數 (Show customers post count)** 核取方塊以啟用顯示顧客所建立的貼文數量。
...
- 勾選 **允許使用者對貼文進行投票** 核取方塊以啟用投票功能。
  - **每日最高投票次數** 欄位用於設定若已啟用上述設定時，使用者每天可進行的投票數量。
- 勾選 **允許私人訊息** 核取方塊以啟用私人訊息功能。若已啟用，下列兩項設定將會顯示：
...
- 勾選 **Active discussions feed enabled** 核取方塊以啟用熱門討論內容頁面的 RSS feeds。
- 在 **Active discussions feed count** 欄位中，設定「熱門討論 (Active discussions)」feed 所包含的討論數量。
