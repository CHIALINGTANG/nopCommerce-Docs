...
論壇是一個線上討論網站，人們可以在其中以張貼訊息的形式進行對話。一個論壇可能包含多個子論壇，每個子論壇又包含數個內容頁面。

> [!NOTE]
>
> 在 nopCommerce 中，論壇預設為停用。若要啟用論壇，請前往 **設定 → 設定 → 論壇設定**，並勾選 **啟用論壇** 核取方塊。啟用後，「論壇」連結應會顯示在前台網站的選單中（預設佈景主題為頂部選單或頁尾）。
...
此頁面支援多商店設定；這意味著可以為所有商店定義相同的設定，或為不同的商店設定不同的值。如果您想要管理特定商店的設定，請從多商店設定下拉式選單中選擇該商店名稱，並勾選左側所需的核取方塊，以針對該商店設定自訂數值。如需進一步詳細資訊，請參閱 [多商店](xref:zh-Hant/getting-started/advanced-configuration/multi-store)。
...
- 勾選 **啟用簽名 (Signature enabled)**，讓顧客有機會設定自己的個人簽名。
...
- 勾選 **允許私人訊息** 核取方塊以啟用私人訊息功能。如果啟用，將會顯示以下兩個設定：
...
- 勾選 **Forum feeds enabled** 核取方塊，為每個論壇啟用 RSS 摘要。
- 在 **Forum feed count** 欄位中，設定每個摘要中要包含的內容頁面數量。
- 勾選 **Active discussions feed enabled** 核取方塊，為活躍討論內容頁面啟用 RSS 摘要。
- 在 **Active discussions feed count** 欄位中，設定要包含在「活躍討論 (Active discussions)」摘要中的討論數量。
