...
論壇是一個線上討論網站，人們可以在其中以發佈訊息的形式進行對話。一個論壇可能包含多個子論壇，每個子論壇又包含數個內容頁面。
...
- 設定新的論壇群組 **名稱**。
- 在 **顯示順序** 欄位中，輸入論壇群組的顯示順序。數值 1 代表顯示在列表的最上方。
...
- 從 **討論區群組** 下拉式選單中，選擇所需的討論區群組。
- 輸入新討論區的 **名稱**。
- 輸入新討論區的 **說明**。
- 選擇討論區的 **顯示順序**。數值 1 代表位於列表最上方。
...
此頁面支援多商店設定；這意味著可以為所有商店定義相同的設定，或是根據不同商店設定不同的值。如果您想要管理特定商店的設定，請從多商店設定下拉式清單中選擇其名稱，並勾選左側所需的核取方塊，以便為其設定自訂值。如需進一步詳細資料，請參考 [多商店](xref:zh-Hant/getting-started/advanced-configuration/multi-store)。
...
- 勾選 **允許使用者對貼文進行投票** 核取方塊以啟用投票功能。
  - **每日最高投票數** 欄位用於設定若啟用了上述設定，使用者每日可進行的投票次數。
- 勾選 **允許私人訊息** 核取方塊以啟用私人訊息功能。若啟用，下列兩項設定將會顯示：
  - 勾選 **顯示私人訊息提示** 核取方塊，以便在收到新私人訊息時啟用提示彈出視窗。
...
在 *Feeds* 面板中設定以下論壇相關設定：

- 勾選 **Forum feeds enabled** 核取方塊，可為各個論壇啟用 RSS 訊息饋送。
- 在 **Forum feed count** 欄位中，設定每個訊息饋送所包含的內容頁面（Topic）數量。
- 勾選 **Active discussions feed enabled** 核取方塊，可為活躍討論內容頁面啟用 RSS 訊息饋送。
- 在 **Active discussions feed count** 欄位中，設定「活躍討論」訊息饋送中所包含的討論數量。
