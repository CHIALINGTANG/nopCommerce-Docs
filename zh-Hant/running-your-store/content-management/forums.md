...
論壇是一個線上討論網站，人們可以在這裡透過發布訊息的形式進行對話。一個論壇可能包含多個子論壇，每個子論壇中又有若干個討論串。
...
此頁面支援多商店設定；這意味著可以為所有商店定義相同的設定，或是針對不同商店設定不同的值。如果您想要管理特定商店的設定，請從多商店設定下拉式選單中選擇該商店名稱，並勾選左側所需的核取方塊，即可為其設定自訂值。若需進一步了解，請參閱 [多商店](xref:zh-Hant/getting-started/advanced-configuration/multi-store)。
...
- 勾選 **Show customers post count** 核取方塊以顯示顧客發布的貼文總數。
- 從 **Forum editor** 下拉選單中，選擇要使用的論壇編輯器類型：
...
- 勾選 **允許私人訊息** 核取方塊以啟用私人訊息功能。若啟用，以下兩項設定將會顯示：
  - 勾選 **顯示私人訊息提醒** 核取方塊，以便在收到新私人訊息時顯示提醒彈出視窗。
  - 若希望在顧客收到新私人訊息時透過電子郵件通知，請勾選 **通知私人訊息**。
...
- 勾選 **Forum feeds enabled** 核取方塊，為每個論壇啟用 RSS feeds。
- 在 **Forum feed count** 欄位中，設定每個 feed 所包含的內容頁面（Topic）數量。
- 勾選 **Active discussions feed enabled** 核取方塊，啟用熱門討論內容的 RSS feeds。
